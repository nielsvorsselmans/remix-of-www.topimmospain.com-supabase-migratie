...
interface ExtrasPdfDownloadProps {
  saleId: string;
  disabled?: boolean;
}

export function ExtrasPdfDownload({ saleId, disabled }: ExtrasPdfDownloadProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('generate-extras-pdf', {
        body: { saleId }
      });
      ...
      if (data?.cached && data?.url) {
        // Fetch cached HTML from storage and write to window
        console.log('Using cached extras PDF:', data.url);
        try {
        ...
        // Fallback: write fresh HTML to window
        printWindow.document.open();
      ...
      toast.success('Extra\'s PDF geopend');
    } catch (error) {
