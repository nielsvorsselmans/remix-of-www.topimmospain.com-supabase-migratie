-- Stap 1: Voeg linked_visitor_ids kolommen toe aan customer_profiles en crm_leads
ALTER TABLE public.customer_profiles 
ADD COLUMN IF NOT EXISTS linked_visitor_ids TEXT[] DEFAULT '{}';

ALTER TABLE public.crm_leads 
ADD COLUMN IF NOT EXISTS linked_visitor_ids TEXT[] DEFAULT '{}';

-- Stap 2: Migreer bestaande visitor_id data naar linked_visitor_ids arrays
UPDATE public.customer_profiles 
SET linked_visitor_ids = ARRAY[visitor_id]
WHERE visitor_id IS NOT NULL 
  AND (linked_visitor_ids IS NULL OR linked_visitor_ids = '{}');
...
-- Stap 3: Vul linked_visitor_ids vanuit alle tracking_events voor users
UPDATE public.customer_profiles cp
SET linked_visitor_ids = subquery.all_visitor_ids
FROM (
  SELECT 
    te.user_id,
    ARRAY_AGG(DISTINCT te.visitor_id) FILTER (WHERE te.visitor_id IS NOT NULL) as all_visitor_ids
  FROM public.tracking_events te
  WHERE te.user_id IS NOT NULL
  GROUP BY te.user_id
) subquery
WHERE cp.user_id = subquery.user_id
  AND subquery.all_visitor_ids IS NOT NULL
  AND array_length(subquery.all_visitor_ids, 1) > 0;

-- Stap 4: Vul linked_visitor_ids vanuit alle tracking_events voor CRM leads
...
FROM (
  SELECT 
    ...
    ARRAY_AGG(DISTINCT te.visitor_id) FILTER (WHERE te.visitor_id IS NOT NULL) as all_visitor_ids
  FROM public.tracking_events te
  WHERE te.crm_user_id IS NOT NULL
  GROUP BY te.crm_user_id
) subquery
WHERE cl.crm_user_id = subquery.crm_user_id
  AND subquery.all_visitor_ids IS NOT NULL
  AND array_length(subquery.all_visitor_ids, 1) > 0;

-- Stap 5: Update aggregate_customer_profile functie voor multi-device aggregatie
