-- Update auto_create_journey_milestones trigger to use ON CONFLICT DO UPDATE for customer_profiles
...
BEGIN
  ...
  -- UPDATED: Use ON CONFLICT DO UPDATE to prevent constraint errors and update user_id if provided
  ...
  ON CONFLICT (crm_lead_id) DO UPDATE SET
    ...
    crm_user_id = COALESCE(EXCLUDED.crm_user_id, customer_profiles.crm_user_id),
    updated_at = NOW();
