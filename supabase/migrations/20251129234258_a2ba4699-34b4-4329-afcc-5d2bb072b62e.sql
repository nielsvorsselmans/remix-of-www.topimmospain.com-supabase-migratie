-- Update aggregate_customer_profile function to calculate new engagement metrics
...
BEGIN
  ...
  -- Aggregate engagement metrics from tracking_events with NEW metrics
  SELECT jsonb_build_object(
    ...
    'total_blog_views', COUNT(*) FILTER (WHERE event_name IN ('page_view', 'blog_view') AND event_params->>'blog_post_id' IS NOT NULL),
    ...
    'avg_time_per_project', COALESCE(
      AVG(time_spent_seconds) FILTER (WHERE time_spent_seconds > 0 AND path LIKE '/projecten/%' AND path NOT LIKE '/projecten/gemeente%'), 
      0
    ),
