-- Fix Blog Views Counting and Project Time Path in aggregate_customer_profile
...
BEGIN
  ...
  -- Aggregate engagement metrics from tracking_events with FIXED blog and project path counting
    ...
    'repeat_project_views', (
      SELECT COUNT(*) FROM (
        ...
        WHERE ((te.user_id = p_user_id AND p_user_id IS NOT NULL)
          OR (te.visitor_id = p_visitor_id AND p_visitor_id IS NOT NULL)
...
-- Re-aggregate all existing customer profiles
DO $$
DECLARE
  profile RECORD;
...
BEGIN
  ...
  SELECT COUNT(*) INTO total_count FROM customer_profiles;
  
  RAISE NOTICE 'Starting re-aggregation of % customer profiles...', total_count;
  
  -- Loop through all customer profiles and re-aggregate
  FOR profile IN 
    SELECT visitor_id, user_id, crm_user_id 
    FROM customer_profiles
  LOOP
    PERFORM aggregate_customer_profile(profile.user_id, profile.visitor_id, profile.crm_user_id);
    processed_count := processed_count + 1;
    
    -- Log progress every 50 profiles
    IF processed_count % 50 = 0 THEN
      RAISE NOTICE 'Processed % / % profiles', processed_count, total_count;
    END IF;
  END LOOP;
  
  RAISE NOTICE 'Re-aggregation complete! Processed % profiles.', processed_count;
END $$;