-- Phase 2.1: Enhance aggregate_customer_profile to process filter_applied events
...
BEGIN
  ...
  -- Get viewed projects (from page_view events)
  ...
  -- Get favorited projects (from project_favorited events)
  SELECT ARRAY_AGG(DISTINCT (event_params->>'project_id')::uuid)
  ...
  -- Infer preferences from BOTH viewed projects AND filter_applied events
  ...
  filter_stats AS (
    -- Extract budget and region info from filter_applied events
    SELECT
