-- Drop and recreate klant_detail_view with partner information
DROP VIEW IF EXISTS klant_detail_view;

CREATE VIEW klant_detail_view AS
SELECT 
  cl.id,
  cl.crm_user_id,
  cl.email,
  ...
  cl.recontact_after,
  cl.merged_at,
  -- Customer profile data (keep original column order)
  ...
  -- Partner info from join (added at end to avoid column order issues)
  p.id as partner_id,
  p.name as partner_name,
  p.company as partner_company,
  p.logo_url as partner_logo_url,
  -- Assigned projects as JSONB
  (
