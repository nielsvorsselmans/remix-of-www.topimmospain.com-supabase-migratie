...
BEGIN
  ...
  ELSIF v_overd_total > 0 AND v_overd_done < v_overd_total THEN
    v_status := 'overdracht';
  ELSIF v_overd_total > 0 AND v_overd_done = v_overd_total THEN
    -- Alle overdracht-taken klaar: check of notarisdatum is gepasseerd
    IF v_notary_date IS NOT NULL AND v_notary_date <= CURRENT_DATE THEN
      -- Notarisdatum bereikt: door naar nazorg of afgerond
      ...
      END IF;
    ELSE
      -- Notarisdatum niet bereikt of niet ingesteld: blijf op overdracht
      v_status := 'overdracht';
    END IF;
  ELSE
    v_status := 'reservatie';
