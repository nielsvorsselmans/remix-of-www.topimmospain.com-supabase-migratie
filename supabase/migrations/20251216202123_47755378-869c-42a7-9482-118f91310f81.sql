...
-- Fix: Update function to use correct column name template_key instead of milestone_key
...
DECLARE
  v_template_key TEXT;
BEGIN
  -- Map document_type to template key
...
BEGIN
  ...
  IF NEW.paid_at IS NOT NULL AND (OLD IS NULL OR OLD.paid_at IS NULL) THEN
    ...
    IF v_payment_index = 1 THEN
      ...
      SET completed_at = COALESCE(completed_at, NOW()), updated_at = NOW()
      WHERE sale_id = v_sale_id
        AND template_key = 'res_aanbetaling'
        AND completed_at IS NULL;
    END IF;
  END IF;
...
BEGIN
  ...
  SET completed_at = COALESCE(completed_at, NOW()), updated_at = NOW()
  WHERE sale_id = NEW.sale_id
    AND template_key = 'res_betaalplan'
    AND completed_at IS NULL;
...
BEGIN
  ...
  IF v_all_complete THEN
    ...
    SET completed_at = COALESCE(completed_at, NOW()), updated_at = NOW()
    WHERE sale_id = v_sale_id
      AND template_key = 'res_koperdata'
      AND completed_at IS NULL;
  END IF;
...
DECLARE
  v_template_key TEXT;
BEGIN
  v_template_key := CASE NEW.approval_type
  ...
  IF v_template_key IS NOT NULL AND NEW.approved_at IS NOT NULL THEN
    UPDATE public.sale_milestones
    SET completed_at = COALESCE(completed_at, NOW()), updated_at = NOW()
    WHERE sale_id = NEW.sale_id
      AND template_key = v_template_key
...
BEGIN
  IF NEW.status = 'completed' AND (OLD IS NULL OR OLD.status != 'completed') THEN
    ...
    SET completed_at = COALESCE(completed_at, NOW()), updated_at = NOW()
    WHERE sale_id = NEW.sale_id
      AND template_key = 'overd_snagging'
      AND completed_at IS NULL;
  END IF;
